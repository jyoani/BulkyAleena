...
unitOfWork.shoppingCartRepository.GetAll(u => u.ApplicationUserId == userId).Count());
        ...
        public IActionResult FilterCol(string Color)
        {
    ...
    .Where(product => product.DressColor == Color).ToList();
        ...
        public IActionResult FilterColorss()
        {
            IEnumerable<Product> productList = unitOfWork.product
                .GetAll(includeProperties: "Category,ProductImages")
                .Where(product => product.DressColor == "Blue")
                .ToList();

            return View(productList);
        }

        public IActionResult FilterColorcrct()
        {
            IEnumerable<Product> productList = unitOfWork.product
    .GetAll(includeProperties: "Category")
